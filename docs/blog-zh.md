...
*左：Transformer——每个 token 都要看所有其他 token，像一个巨大的关系网，越长越慢。中：Mamba——信息像流水一样往前传，快但容易丢细节。右：Hybrid——两者结合，既快又准。*
...
手动调参太慢了。我接入了一个叫 autoresearch 的框架——简单说就是：
...
*橙色：矩阵乘法。蓝色：并行扫描。小块（64/128）时矩阵乘法更快，大块（512）时并行扫描胜出。在 256 附近打平。*
...
*问 "The capital of France is"，修复前模型输出的是 "amo"、"imore" 这些无意义的词（左），修复后 "Paris" 排名第一（右）。*
...
*蓝色柱子是速度（越高越好），橙色柱子是内存占用（越低越好）。4-bit 量化后，速度达到 66.7 tok/s，内存仅 1.3 GB。*
